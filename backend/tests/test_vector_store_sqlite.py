...
def test_sqlite_vector_store_deletes_by_metadata(tmp_path) -> None:
    store = SQLiteVectorStore(f"sqlite:///{tmp_path / 'vectors.db'}")
    store.add_documents(
        [
            VectorDocument(
                ...
                text="First document",
                embedding=[1.0, 0.0],
                metadata={"document_id": "doc-1", "group_id": "alpha"},
            ),
            VectorDocument(
                id="second",
                text="Second document",
                embedding=[1.0, 0.0],
                metadata={"document_id": "doc-2", "group_id": "alpha"},
            ),
        ]
    )

    store.delete_by_metadata({"document_id": "doc-1", "group_id": "alpha"})
    results = store.search([1.0, 0.0], metadata_filter={"group_id": "alpha"})
